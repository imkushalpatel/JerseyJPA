...
public class JettyFactory {
	...
	public Server build() throws Exception {
		...
		// Since we don't package the project as a war, we use our
		// src/main/webapp directory
		// as the resource base directory for the server (will serve static
		// content from the webapp directory)
		...
		// The annotationConfiguration is required to support annotations like
		// @WebServlet
		// See:
		// http://www.eclipse.org/jetty/documentation/current/configuring-webapps.html
		webapp.setConfigurations(new Configuration[] { new AnnotationConfiguration() // Scan
																						// container
																						// and
																						// web
																						// app
				...
				, new MetaInfConfiguration() // Looks in container and webapp
												// jars for META-INF/resources
												// and META-INF/web-fragment.xml
				...
				, new PlusConfiguration() // Processes JNDI related aspects of
											// WEB-INF/web.xml and hooks up
											// naming entries
				, new JettyWebXmlConfiguration() // Processes a
													// WEB-INF/jetty-web.xml
													// file
		});

		// Important! make sure Jetty scans all classes under ./classes looking
		// for annotations.
