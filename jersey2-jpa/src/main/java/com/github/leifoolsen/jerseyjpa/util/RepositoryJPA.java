...
public class RepositoryJPA implements Repository {
	...
	public <T> T createOrUpdate(final T entity) {
		...
		try {
			...
			if (logger.isDebugEnabled())
				logger.debug(logMsg, entityName(entity.getClass()), id);
			return result;
		} catch (RuntimeException e) {
			if (!transactionAlreadyStarted) {
				em.getTransaction().rollback();
	...
	public <T> void remove(final Class<T> entityClass, final Object id) {
		...
		if (entity != null) {
			...
			try {
				...
				transactionAlreadyStarted = true; // block commit in finally
													// block
				throw e;
			} finally {
				if (!transactionAlreadyStarted) {
			...
			logger.debug("Nothing to remove. Entity: '{}' with id: '{}' was not found.", entityClass.getSimpleName(),
					id);
		}
	}
	...
	public <T> List<T> find(final Class<T> entityClass, final Integer offset, final Integer limit) {
		...
		TypedQuery<T> query = getEntityManager().createQuery(String.format("select e from %s e", entityName),
				entityClass);
	...
	/**
	 * WARNIG! Not an efficient count-query. Use this as a template to create
	 * your own count. e.g.; "select count(e.id) from MyEntity e"
	 */
	@Override
	public <T> long count(final Class<T> entityClass) {
		final String entityName = entityName(entityClass);
		final Long result = (Long) getEntityManager().createQuery("select count(e) from " + entityName + " e")
				.getSingleResult();
	...
	@Override
	public <T> TypedQuery<T> createNamedQuery(final String queryName, final Class<T> resultClass,
			final Map<String, Object> parameters) {
	...
	@Override
	public Query createNativeQuery(final String sql, final Class resultClass, final Map<String, Object> parameters) {
	...
	// TODO: Make fluent
	// e.g:
	// Members.ofType(Field.class).and(Method.class).annotatedWith(Id.class).or(Foo.class).in(MyClass.class).get();
	public static <T> List<Member> findMembersAnnotatedWith(final Class<T> target,
	...
	public static Object getMemberValue(final Member member, final Object target) {
		return member instanceof Field ? getFieldValue((Field) member, target)
				: getMethodValue((Method) member, target);
	}
	...
	public static Object getMethodValue(final Method method, final Object target) {
		...
		try {
			...
			throw new IllegalArgumentException("Not a valid getter method: %s" + methodToString(method), e);
		} finally {
