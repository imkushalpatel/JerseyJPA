...
public class CollectionJson {
	...
	protected CollectionJson(Collection collection) {
		this.collection = collection;
	}

	protected CollectionJson(Template template) {
	...
	/*
	 * Item Representation: An item response will usually look like a collection
	 * representation, but contain only one item. The server MAY not return the
	 * queries or template properties within a response, but include annotated
	 * links instead.
	 * 
	 ...
	 * 
	 * "items" : [ { "href" : "http://example.org/friends/jdoe", "data" : [
	 * {"name" : "full-name", "value" : "J. Doe", "prompt" : "Full Name"},
	 * {"name" : "email", "value" : "jdoe@example.org", "prompt" : "Email"} ],
	 * "links" : [ {"rel" : "blog", "href" : "http://examples.org/blogs/jdoe",
	 * "prompt" : "Blog"}, {"rel" : "avatar", "href" :
	 * "http://examples.org/images/jdoe", "prompt" : "Avatar", "render" :
	 * "image"} ] } ] } }
	 */
	...
	/*
	 * The links array is an OPTIONAL child property of the items array. It
	 * SHOULD contain one or more anonymous objects. Each has five possible
	 * properties: href (REQUIRED), rel (REQURIED), name (OPTIONAL), render
	 * (OPTIONAL), and prompt, (OPTIONAL).
	 * 
	 * Sample links array:
	 * 
	 * { "collection" : { "version" : "1.0", "href" : URI, "items" : [ { "href"
	 * : URI, "data" : [ARRAY], "links" : [ {"href" : URI, "rel" : STRING,
	 * "prompt" : STRING, "name" : STRING, "render" : "image"}, {"href" : URI,
	 * "rel" : STRING, "prompt" : STRING, "name" : STRING, "render" : "link"},
	 ...
	 * collection "records." This is an OPTIONAL object and there MUST NOT be
	 * more than one template object in a Collection+JSON document. It is a
	 * top-level document property.
	 * 
	 * The template object SHOULD have a data array child property. { "template"
	 * : { "data" : [ARRAY] } }
	 * 
	 * 
	 * Template Representation: To reduce the size of the response
	 * representation, servers MAY return a link to the template object for a
	 * collection. Clients can then request the template representation
	 * directly.
	 * 
	 * { "collection" : { "version" : "1.0", "href" :
	 * "http://example.org/friends/",
	 * 
	 * "template" : { "data" : [ {"name" : "full-name", "value" : "", "prompt" :
	 * "Full Name"}, {"name" : "email", "value" : "", "prompt" : "Email"},
	 * {"name" : "blog", "value" : "", "prompt" : "Blog"}, {"name" : "avatar",
	 * "value" : "", "prompt" : "Avatar"} ] } } }
	 * 
	 * Template Write Representation: When sending data to the server, clients
	 * should fill in the template object and use that as the body of an HTTP
	 * POST (for "create") or HTTP PUT ("update") request.
	 * 
	 * { "template" : { "data" : [ {"name" : "full-name", "value" :
	 * "W. Chandry"}, {"name" : "email", "value" : "wchandry@example.org"},
	 * {"name" : "blog", "value" : "http://example.org/blogs/wchandry"}, {"name"
	 * : "avatar", "value" : "http://example.org/images/wchandry"} ] } }
	 */
	 ...
	 * 
	 * The queries array SHOULD contain one or more anonymous objects. Each
	 * object composed of five possible properties: href (REQUIRED), rel
	 * (REQUIRED), name (OPTIONAL), prompt (OPTIONAL), and a data array
	 * (OPTIONAL).
	 * 
	 * If present, the data array represents query parameters for the associated
	 * href property of the same object. See Query Templates for details.
	 * 
	 * { "queries" : [ {"href" : URI, "rel" : STRING, "prompt" : STRING, "name"
	 * : STRING}, {"href" : URI, "rel" : STRING, "prompt" : STRING, "name" :
	 * STRING, "data" : [ {"name" : STRING, "value" : VALUE} ] }, ... {"href" :
	 * URI, "rel" : STRING, "prompt" : STRING, "name" : STRING} ] }
	 * 
	 * 
	 * Queries Representation: To reduce the size of the response
	 * representation, servers MAY return a link to the queries array for a
	 * collection. Clients can then request the queries representation directly.
	 * 
	 * { "collection" : { "version" : "1.0", "href" :
	 * "http://example.org/friends/",
	 * 
	 * "queries" : [ {"rel" : "search", "href" :
	 * "http://example.org/friends/search", "prompt" : "Search" "data" : [
	 * {"name" : "search", "value" : ""} ] } ] } }
	 */
	 ...
	 * condition reported by the server. This is an OPTIONAL object and there
	 * MUST NOT be more than one error object in a Collection+JSON document. It
	 * is a top-level document property.
	 * 
	 * The following elements MAY appear as child properties of the error
	 * object: code message and title.
	 * 
	 * { "error" : { "title" : STRING, "code" : STRING, "message" : STRING } }
	 * 
	 * 
	 * Error Representation: When the server encounters an error, it MAY return
	 * an error object.
	 * 
	 * { "collection" : { "version" : "1.0", "href" :
	 * "http://example.org/friends/",
	 * 
	 * "error" : { "title" : "Server Error", "code" : "X1C2", "message" :
	 * "The server have encountered an error, please wait and try again." } } }
	 */
	...
	public static class Error {
		...
		public <T> T unMarshalError(final Class<T> entityClass) {
			JsonBuilderFactory factory = Json.createBuilderFactory(null);
			JsonObjectBuilder builder = factory.createObjectBuilder().add("title", title).add("code", code)
					.add("message", message);
			JsonObject jsonObject = builder.build();
			return JaxbHelper.unMarshal(entityClass, jsonObject);
		}
