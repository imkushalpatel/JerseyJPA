...
public class BookResource {
	...
	public Response create(@BeanParam final BookDTO params) {
		...
		Book book = Book.with(params.isbn).title(params.title).author(params.author)
				.published(params.published != null ? params.published.getLocalDate() : null)
				.translator(params.translator).summary(params.summary).publisher(publisher).build();
		...
		return Response.created(uriInfo.getAbsolutePathBuilder().clone().path(params.isbn).build())
				.entity(collectionJson).build();
	}
	...
	public Response update(@BeanParam final BookDTO params) {
		...
		Publisher publisher = lookupPublisher(params.publisherCode);
		Book bookToUpdate = Book.with(params.isbn).title(params.title).author(params.author)
				.published(params.published != null ? params.published.getLocalDate() : null)
				.translator(params.translator).summary(params.summary).publisher(publisher)
		...
		return Response.ok(collectionJson).location(uriInfo.getAbsolutePathBuilder().clone().path(params.isbn).build())
				.build();
	}
	...
	public Response byIsbn(@Isbn @PathParam("isbn") final String isbn) {
		...
		if (book == null) {
			throw new WebApplicationException("Book with isbn: '" + isbn + "' was not found",
					Response.status(Response.Status.NOT_FOUND).location(uriInfo.getRequestUri()).build());
		}
		CollectionJson collectionJson = CollectionJsonResourceHelper.buildCollectionJson(uriInfo, book);
		return Response.ok(collectionJson).location(uriInfo.getRequestUri()).build();
	}
	...
	public Response allBooks(@QueryParam("offset") Integer offset, @QueryParam("limit") Integer limit) {
		// Calling SearchResource directly is a hack, I think, but don't know
