...
public class CollectionJsonResourceHelper {
	...
	public static UriBuilder resourceRootUriBuilder(final UriInfo uriInfo) {
		...
		if (path != null)
			uriBuilder.path(path); // e.g. http://example.org/api/books
		return uriBuilder;
	}
	...
	public static CollectionJson.Item bookToCollectionJsonItem(final UriInfo uriInfo, final Book book) {
		...
		item.addData("id", book.getId(), "Id").addData("version", book.getVersion().toString(), "Version")
				.addData("isbn", book.getISBN(), "ISBN").addData("title", book.getTitle(), "Title")
		...
		item.addLink("self", uriBuilder.clone().path(book.getISBN()).toString(), "This book")
				.addLink("publisher", uriBuilder.clone().path(book.getISBN()).path("publisher").build().toString(),
						"Publisher of book")
				.addLink("authorship",
						uriBuilder.clone().path("search/author").queryParam("q", book.getAuthor()).toString(),
	...
	public static CollectionJson.Item publisherToCollectionJsonItem(final UriInfo uriInfo, final Publisher publisher) {
		...
		item.addData("id", publisher.getId(), "Id").addData("version", publisher.getVersion().toString(), "Version")
				.addData("code", publisher.getCode(), "Code").addData("name", publisher.getName(), "Name");

		item.addLink("self", uriInfo.getRequestUri().toString(), "This publisher").addLink("books",
				builder.clone().path(SearchType.Type.get("publisher.code").type()).queryParam("q", publisher.getCode())
						.build().toString(),
	...
	public static List<CollectionJson.Query> collectionJsonQueries(final UriInfo uriInfo) {
		...
		for (SearchType.Type t : SearchType.Type.values()) {
			...
			CollectionJson.Query q = new CollectionJson.Query("search",
					builder.clone().path(t.type()).build().toString(), "Search " + t.type());
