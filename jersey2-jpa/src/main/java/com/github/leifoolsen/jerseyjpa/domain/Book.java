...
public class Book {
	...
	public LocalDate getPublished() {
		return DateLocalDateUtil.dateToLocalDate(published);
	}
	...
	/**
	 * <p>
	 * An ISBN number is divided into the following number segments:
	 * </p>
	 * <ul>
	 * <li>EAN: (European article number) product code: the first three
	 * digits</li>
	 * <li>Group identifier: a single digit following the EAN product code that
	 * specifies the country or language in which the book is published</li>
	 * <li>Publisher prefix: a number that identifies a particular publisher
	 * within the preceding group</li>
	 * <li>Title identifier: a number that identifies a particular title or
	 * edition of a title issued by the preceding publisher</li>
	 * <li>Check digit: a single digit at the end of the ISBN that validates the
	 * accuracy of the ISBN</li>
	 * </ul>
	 * <p>
	 * More details about the ISBN number can be found e.g. here:
	 * </p>
	 * <ul>
	...
	public String formattedISBN() {
		return isbn.substring(0, 3) + "-" + isbn.substring(3, 4) + "-" + isbn.substring(4, 9) + "-"
				+ isbn.substring(9, 12) + "-" + isbn.substring(12);
	}
	...
	public String toString() {
		return "Book:{" + "id='" + id + '\'' + ", version=" + version + ", isbn='" + isbn + '\'' + ", title='" + title
				+ '\'' + ", author='" + author + '\'' + ", published=" + published + ", translator='" + translator
				+ '\'' + ", summary='" + summary + '\'' + ", publisher=" + publisher + '}';
	}

	public static Builder with(final String isbn) {
		return new Builder(isbn);
	}

	public static Builder with(final Book source, final boolean copyId) {
		Builder b = new Builder(source.isbn).title(source.title).author(source.author).published(source.getPublished())
				.summary(source.summary).publisher(source.publisher);
