...
public class SearchResource {
	...
	@GET
	@Compress // Client send request with "Accept-Encoding: gzip" - so
				// annotation is not required
	public Response allBooks(@SearchType @PathParam("searchType") final String searchType,
			@QueryParam("q") final String searchValue, @QueryParam("offset") final Integer offset,
			@QueryParam("limit") final Integer limit) {

		Integer off = Math.max(MoreObjects.firstNonNull(offset, 0), 0); // At
																		// least
																		// zero
		Integer lim = Math.max(MoreObjects.firstNonNull(limit, DEFAULT_LIMIT), 1); // Greater
																					// than
																					// zero
		...
		UriBuilder rootUriBuilder = CollectionJsonResourceHelper.resourceRootUriBuilder(uriInfo).path("search")
				.path(searchType);
		...
		if (off > 0) {
			...
			collectionJson.collection().addLink("prev",
					rootUriBuilder.clone().queryParam("offset", pOffset).queryParam("limit", lim).toString());
		}
		if (books.size() >= lim) {
			int nOffset = off + lim;
			collectionJson.collection().addLink("next",
					rootUriBuilder.clone().queryParam("offset", nOffset).queryParam("limit", lim).toString());
		}
