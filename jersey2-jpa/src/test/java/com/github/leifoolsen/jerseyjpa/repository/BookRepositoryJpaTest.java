...
public class BookRepositoryJpaTest {
	...
	private static JpaDatabaseConnectionManager.JpaDatabaseConnection connection = JpaDatabaseConnectionManager
			.createConnection(PU_NAME);
	...
	public static void beforeClass() {
		// Configure PU //
		Properties properties = PersistenceProperties.createPropertiesForProvider(PersistenceProperties.HIBERNATE, null,
				Arrays.asList(Publisher.class, Book.class));
	...
	public void newBook() {
		...
		Book book = Book.with("9788202289331").publisher(publisher).title("Kurtby").author("Loe, Erlend")
	...
	public void updateBook() {
		...
		// Book is managed! Both instances, 'vredensDuer' and 'vredensDruer',
		// vill update
	...
	public void createNewBookAndUpdateExistingBook() {
		...
		Book aNewBook = Book.with("9780297871934")
				.title("Accidence Will Happen : The Non-Pedantic Guide to English Usage").author("Kamm, Oliver")
				.publisher(publisher).published(LocalDate.of(2015, Month.MARCH, 12))
		...
		Book changedBook = Book.with(bookForUpdate, true).published(LocalDate.of(2014, Month.DECEMBER, 15)).build();
	...
	public void shouldFindAtLeastOneBookByPublisherPicador() {
		final List<Book> books = bookRepository.findBooksBySearchType(SearchType.Type.PUBLISHER_NAME, "picador", null,
				null);
	...
	public void shouldFindAnyBookResultLimitedToFourBooks() {
		final List<Book> books = bookRepository.findBooksBySearchType(SearchType.Type.ANY, null, 2, 4);
		assertThat(books, hasSize(4));
	}

	@Test
	public void shouldFindAnyBookWithTextHawking() {
		final List<Book> books = bookRepository.findBooksBySearchType(SearchType.Type.ANY, "hawking", null, null);
		assertThat(books, hasSize(greaterThan(1)));
